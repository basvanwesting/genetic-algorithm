...
#[derive(Clone, Debug)]
pub struct Builder<G: Genotype, F: Fitness<Genotype = G>, S: Crossover, E: Extension> {
...
impl<G: Genotype, F: Fitness<Genotype = G>, S: Crossover, E: Extension> Builder<G, F, S, E> {
    pub fn new() -> Self {
        Self::default()
    }

    pub fn build(self) -> Result<Evolve<G, F, S, E>, TryFromBuilderError> {
        self.try_into()
    }

    pub fn call<R: Rng>(self, rng: &mut R) -> Result<Evolve<G, F, S, E>, TryFromBuilderError> {
        let mut evolve: Evolve<G, F, S, E> = self.try_into()?;
        evolve.call(rng);
        Ok(evolve)
    }
    pub fn call_repeatedly<R: Rng>(
        ...
        max_repeats: usize,
        rng: &mut R,
    ) -> Result<Evolve<G, F, S, E>, TryFromBuilderError> {
        let mut best_evolve: Option<Evolve<G, F, S, E>> = None;
        for iteration in 0..max_repeats {
            log::info!("### repeated round: {}", iteration);
            let mut contending_run: Evolve<G, F, S, E> = self.clone().try_into()?;
    ...
    pub fn call_speciated<R: Rng>(
        ...
        let best_chromosomes: Vec<Chromosome<G>> = (0..number_of_species)
            .filter_map(|iteration| {
                ...
                let mut species_run: Evolve<G, F, S, E> = self.clone().try_into().ok()?;
        ...
        let mut final_run: Evolve<G, F, S, E> =
    ...
    pub fn with_compete(mut self, compete: Compete) -> Self {
...
impl<G: Genotype, F: Fitness<Genotype = G>, S: Crossover, E: Extension> Default
    for Builder<G, F, S, E>
{
    fn default() -> Self {
        Self {
