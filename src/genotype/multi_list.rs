...
use crate::chromosome::{Chromosome, ChromosomeManager, MultiListChromosome, OwnsGenes};
...
impl<T: Allele + PartialEq> ChromosomeManager<Self> for MultiList<T> {
    ...
    fn chromosome_bin_push(&mut self, mut chromosome: MultiListChromosome<T>) {
        chromosome.reset();
