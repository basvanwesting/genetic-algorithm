...
#[derive(Debug, Clone)]
pub struct Discrete<T: Default + Clone + std::fmt::Debug> {
    pub gene_size: usize,
...
impl<T: Default + Clone + std::fmt::Debug> TryFrom<Builder<Self>> for Discrete<T> {
...
impl<T: Default + Clone + std::fmt::Debug> Genotype for Discrete<T> {
    type Gene = T;
...
impl<T: Default + Clone + std::fmt::Debug> PermutableGenotype for Discrete<T> {
    fn gene_values(&self) -> Vec<Self::Gene> {
        self.gene_values.clone()
    }
}

impl<T: Default + Clone + std::fmt::Debug> fmt::Display for Discrete<T> {
