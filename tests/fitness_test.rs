...
mod fitness_tests {
    ...
    fn test_simple_sum_binary() {
        ...
        assert_eq!(fitness::SimpleSum.call_for_chromosome(&chromosome), 3);

        let chromosome = build::chromosome::<BinaryGenotype>(vec![true, false, true]);
        assert_eq!(fitness::SimpleSum.call_for_chromosome(&chromosome), 2);

        let chromosome = build::chromosome::<BinaryGenotype>(vec![true, false, false]);
        assert_eq!(fitness::SimpleSum.call_for_chromosome(&chromosome), 1);

        let chromosome = build::chromosome::<BinaryGenotype>(vec![false, false, false]);
        assert_eq!(fitness::SimpleSum.call_for_chromosome(&chromosome), 0);
    }

    #[test]
    fn test_simple_sum_discrete() {
        let chromosome = build::chromosome::<DiscreteGenotype<DiscreteGene>>(vec![1, 2, 3]);
        assert_eq!(fitness::SimpleSum.call_for_chromosome(&chromosome), 6);
    ...
    fn test_simple_sum_continuous() {
        ...
        assert_eq!(fitness::SimpleSum.call_for_chromosome(&chromosome), 7);
