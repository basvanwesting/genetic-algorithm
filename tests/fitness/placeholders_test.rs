...
fn count_true() {
    ...
    assert_eq!(CountTrue.call_for_chromosome(&chromosome), Some(3));

    let chromosome = build::chromosome::<BinaryGenotype>(vec![true, false, true]);
    assert_eq!(CountTrue.call_for_chromosome(&chromosome), Some(2));

    let chromosome = build::chromosome::<BinaryGenotype>(vec![true, false, false]);
    assert_eq!(CountTrue.call_for_chromosome(&chromosome), Some(1));

    let chromosome = build::chromosome::<BinaryGenotype>(vec![false, false, false]);
    assert_eq!(CountTrue.call_for_chromosome(&chromosome), Some(0));
}
...
fn sum_continuous_genotype() {
    ...
    assert_eq!(
        SumContinuousGenotype.call_for_chromosome(&chromosome),
        Some(0)
    );
