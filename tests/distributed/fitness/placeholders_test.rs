...
fn binary_genotype() {
    let genotype = BinaryGenotype::builder()
        ...
        .build()
        .unwrap();
    let chromosome: VecChromosome<bool> = build::chromosome(vec![true, true, true]);
    assert_eq!(
    ...
    let chromosome: VecChromosome<bool> = build::chromosome(vec![true, false, true]);
    assert_eq!(
        CountTrue.calculate_for_chromosome(&chromosome, &genotype),
        Some(2)
    );

    let chromosome: VecChromosome<bool> = build::chromosome(vec![true, false, false]);
    assert_eq!(
        CountTrue.calculate_for_chromosome(&chromosome, &genotype),
        Some(1)
    );

    let chromosome: VecChromosome<bool> = build::chromosome(vec![false, false, false]);
    assert_eq!(
        CountTrue.calculate_for_chromosome(&chromosome, &genotype),
        Some(0)
    );
...
fn list_genotype_u8() {
    ...
    let chromosome: VecChromosome<u8> = build::chromosome(vec![0, 1, 2, 3]);
    assert_eq!(
        Zero::new().calculate_for_chromosome(&chromosome, &genotype),
        Some(0)
    );
    ...
    let chromosome: VecChromosome<u8> = build::chromosome(vec![0, 0, 0, 0]);
    assert_eq!(
        SumGenes::new().calculate_for_chromosome(&chromosome, &genotype),
        Some(0)
    );
...
fn list_genotype_i8() {
    ...
    let chromosome: VecChromosome<i8> = build::chromosome(vec![-2, -1, 0, 1, 2, 3]);
    assert_eq!(
        SumGenes::new().calculate_for_chromosome(&chromosome, &genotype),
        Some(3)
    );

    let chromosome: VecChromosome<i8> = build::chromosome(vec![0, 0, 0, 0]);
    assert_eq!(
        SumGenes::new().calculate_for_chromosome(&chromosome, &genotype),
        Some(0)
    );
...
fn range_genotype_f32() {
    ...
    let chromosome: VecChromosome<f32> = build::chromosome(vec![0.0_f32, 0.0_f32, 0.0_f32]);
    assert_eq!(
        SumGenes::new().calculate_for_chromosome(&chromosome, &genotype),
        Some(0)
    );

    let chromosome: VecChromosome<f32> = build::chromosome(vec![0.1_f32, 0.2_f32, 0.3_f32]);
    assert_eq!(
        SumGenes::new().calculate_for_chromosome(&chromosome, &genotype),
        Some(0)
    );

    let chromosome: VecChromosome<f32> = build::chromosome(vec![1.4_f32, 2.4_f32, 3.4_f32]);
    assert_eq!(
        SumGenes::new().calculate_for_chromosome(&chromosome, &genotype),
        Some(7)
    );
...
fn range_genotype_f64() {
    ...
    let chromosome: VecChromosome<f64> = build::chromosome(vec![0.0_f64, 0.0_f64, 0.0_f64]);
    assert_eq!(
        SumGenes::new_with_precision(1e-3).calculate_for_chromosome(&chromosome, &genotype),
        Some(0)
    );

    let chromosome: VecChromosome<f64> = build::chromosome(vec![0.1_f64, 0.2_f64, 0.3_f64]);
    assert_eq!(
        SumGenes::new_with_precision(1e-3).calculate_for_chromosome(&chromosome, &genotype),
        Some(600)
    );

    let chromosome: VecChromosome<f64> = build::chromosome(vec![1.4_f64, 2.4_f64, 3.4_f64]);
    assert_eq!(
        SumGenes::new_with_precision(1e-3).calculate_for_chromosome(&chromosome, &genotype),
        Some(7199)
    );
